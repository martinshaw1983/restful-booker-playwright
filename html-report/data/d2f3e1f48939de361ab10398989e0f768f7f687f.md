...
- Name: reports.spec.ts >> Verify title
- Location: tests/reports.spec.ts:11:5
...
```ts
  ...
  2  | 
  3  | test.beforeEach('Launch app', async ({ page }) => {
  4  |     await page.goto('https://demowebshop.tricentis.com/');
  5  | });
  6  | 
  7  | test('Verify logo', async ({ page }) => {
  8  |     await expect(page.getByAltText('Tricentis Demo Web Shop')).toBeVisible();
  9  | });
  10 | 
  11 | test('Verify title', async ({ page }) => {
> 12 |     expect(await page.title()).toContain('Demo Web Shop1');
     |                                ^ Error: expect(received).toContain(expected) // indexOf
  13 | });
  14 | 
  15 | test('Verify search', async ({ page }) => {
  16 |     await page.locator('#small-searchterms').fill('laptop');
  17 |     await page.locator('input[type="submit"]').click();
  18 |     await expect.soft(page.locator('.product-title').first()).toContainText('laptop', { ignoreCase: true });
  19 | });
```